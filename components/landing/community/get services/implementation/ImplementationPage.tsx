...
type PlanId = "starter" | "basic" | "standard" | "custom" | "pro";
...
export default function PricingPacksPage() {
  const t = useTranslations("pages.community.implementation");
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const plans = (t.raw("plans") as Plan[]).map((plan, index) => ({
    ...plan,
    color:
      index === 0
        ? "bg-[#d86d3f]"
  ...
  const notes = t.raw("table.notes") as string[];
            ...
            <tbody>
              ...
              {features.map((feature, idx) => (
                <tr key={feature} className="border-b border-border hover:bg-muted/30">
                  ...
                  {plans.map((plan, i) => {
                    let showCheck = false;
                    if (plan.id === "starter" && idx <= 5) showCheck = true;
                    if (plan.id === "basic" && idx <= 6) showCheck = true;
                    if (plan.id === "standard" && idx <= 7) showCheck = true;
                    if (plan.id === "custom" && idx <= 8) showCheck = true;
                    if (plan.id === "pro" && idx <= 8) showCheck = true;
                    if (feature === features[8] && plan.id === "starter")
                      showCheck = true;
                    if (feature === features[8] && plan.id === "basic")
                      showCheck = true;
                    if (feature === features[8] && plan.id === "standard")
                      showCheck = true;
                    if (feature === features[8] && plan.id === "custom")
                      showCheck = true;
                    if (feature === features[8] && plan.id === "pro")
                      showCheck = true;
              ...
              <tr>
                <td className="px-6 py-5 font-bold text-foreground" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
                  {t("table.newCustomers")} <span className="text-sm font-normal text-muted-foreground">{t("table.newCustomersNote")}</span>
                </td>
                {plans.map((plan) => (
                  <td
                    key={plan.name}
      ...
      <section className="bg-muted/30 py-20">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            />
            <ScriptHeading className="mt-4">{t("faqSection.title")}</ScriptHeading>
